...
アプリケーション開発においては、関数型ドメインモデリングを用いた設計については長年の実務経験があり、TypeScriptやScalaを中心としたバックエンド開発において、ドメイン駆動設計と関数型プログラミングの設計手法を実践・体系化してきました。  
データエンジニアリング領域では、データプラットフォームの基盤構築からETL設計、データオブザーバビリティの設計まで、幅広い領域で経験を積んできました。  
また、現職はアーキテクトとして100程度のアーキテクチャのレビューを担当し、システムの分割統治やそれを実現するアーキテクチャスタイルの選定などに貢献しています。

## 📖 書籍

- 執筆中: TypeScript x 関数型ドメインモデリング (2026年内に出版予定)
